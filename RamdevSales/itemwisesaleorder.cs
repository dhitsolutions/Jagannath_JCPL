            ...
            try
            {
                    ...
                    for (int j = 0; j < items.Rows.Count; j++)
                    {
                        ...
                        if (Convert.ToDouble(qty) > 0)
                        {
                            ...
                            if (iscolexist == false)
                            {
                                ...
                                {
                                    maindt.Columns.Add(bills.Rows[i]["billno"].ToString() + Environment.NewLine + bills.Rows[i]["cusname"].ToString());
                                }
                                else
                                {
                ...
                for (int i = 0; i < items.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < bills.Rows.Count; j++)
                    {
                        ...
                        {
                            dr[bills.Rows[j]["billno"].ToString() + Environment.NewLine + bills.Rows[j]["cusname"].ToString()] = Convert.ToDouble(qty).ToString("0.##");
                        }
                        else
                        {
                            dr[bills.Rows[j]["billno"].ToString() + Environment.NewLine + bills.Rows[j]["AccountName"].ToString()] = Convert.ToDouble(qty).ToString("0.##");
                        }
