...
namespace RamdevSales
{
    public partial class SaleReturn : Form
    {
        ...
        public SaleReturn(DateWiseReport dateWiseReport)
        {
            InitializeComponent();
        ...
        public SaleReturn(Ledger ledger)
        {
            InitializeComponent();
            // TODO: Complete member initialization
            this.ledger = ledger;
        ...
        public SaleReturn(DatewiseSaleReturn datewiseSaleReturn)
        {
            InitializeComponent();
            // TODO: Complete member initialization
            this.datewiseSaleReturn = datewiseSaleReturn;
            loadcurrency();
        }
        ...
        void getsr()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("select max(Bill_No) from BillMaster where isactive='1' and billtype='SR' and CompanyId="+Master.companyId+"", con);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        ...
                        {
                            SqlCommand cmd2 = new SqlCommand("Update billproductmaster set isactive='0' where bill_no='" + TxtBillNo.Text + "' and BillType='SR'", con);
                            ...
                            if (dr1 == DialogResult.Yes)
                            {
                                ...
                                //    cmd.ExecuteNonQuery();
                                //}
                                //SqlCommand cmd7 = new SqlCommand("select * from Company where CompanyID='" + Master.companyId + "' and isActive=1", con);
                                //SqlDataAdapter sda7 = new SqlDataAdapter(cmd7);
                                //DataTable dt5 = new DataTable();
                                //sda7.Fill(dt5);
                                //string query = "update Printing set [T43]='" + dt5.Rows[0]["CompanyName"].ToString() + "',[T44]='" + dt5.Rows[0]["Address"].ToString() + "',[T45]=" + dt5.Rows[0]["Phone"].ToString() + ",[T46]='" + dt5.Rows[0]["VATNo"].ToString() + "' where [T2]='" + TxtBillNo.Text + "'";
                                //cmd = new SqlCommand(query, con);
                                //cmd.ExecuteNonQuery();

                                //SaleReturnReport frm = new SaleReturnReport();
                        ...
                        else
                        {
                            ...
                            SqlCommand cmd = new SqlCommand("INSERT INTO [dbo].[BillMaster]([Bill_No],[Bill_Date],[Terms],[ClientID],[PO_No],[SaleType] ,[count],[totalqty],[totalbasic],[totaltax],[totalnet],[isactive],[apprweight],[dispatchdetails],[remarks],[billtype],[CompanyId])VALUES('" + TxtBillNo.Text + "','" + Convert.ToDateTime(TxtRundate.Text).ToString("dd-MMM-yyyy") + "','" + cmbterms.Text + "','" + cmbcustname.SelectedValue + "','" + txtpono.Text + "','" + cmbsaletype.SelectedValue + "'," + lbltotcount.Text + "," + lbltotpqty.Text + "," + lblbasictot.Text + "," + lbltaxtot.Text + "," + Math.Round(Convert.ToDouble(TxtBillTotal.Text), 2).ToString("########.00") + ",1,'" + txtweight.Text + "','" + txtdispatch.Text + "','" + txtremarks.Text + "','SR'," + Master.companyId + ")", con);
                            cmd.ExecuteNonQuery();
        ...
        int cnt = 0;
        private DatewiseSaleReturn datewiseSaleReturn;
        internal void updatemode(string str, string p, int p_2)
        {
            try
            {
                loadpage();
                cnt = 1;
                SqlCommand cmd = new SqlCommand("select * from billmaster where bill_no='" + p + "' and isactive=1 and billtype='SR' and [CompanyId]=" + Master.companyId + "", con);
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                
                SqlCommand cmd1 = new SqlCommand("select * from billproductmaster where bill_no='" + p + "' and billtype='SR' and isactive=1", con);
                SqlDataAdapter sda1 = new SqlDataAdapter(cmd1);
                DataTable dt1 = new DataTable();
                sda1.Fill(dt1);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    TxtRundate.Text = Convert.ToDateTime(dt.Rows[0][1].ToString()).ToString("dd/MMM/yyyy");
        ...
        private void btndelete_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult dr1 = MessageBox.Show("Do you want to Delete Sale Return?", "Bill", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dr1 == DialogResult.Yes)
                {
                    this.Enabled = false;
                    if (con.State == ConnectionState.Open)
                    ...
                    con.Open();
                    SqlCommand cmd = new SqlCommand("update billmaster set isactive=0  where bill_no='" + TxtBillNo.Text + "' and billtype='SR' and [CompanyId]=" + Master.companyId + "", con);
                    cmd.ExecuteNonQuery();

                    SqlCommand cmd2 = new SqlCommand("update billproductmaster set isactive=0 where bill_no='" + TxtBillNo.Text + "' and BillType='SR'", con);
                    cmd2.ExecuteNonQuery();

                    SqlCommand cmd3 = new SqlCommand("update ledger set isactive=0 where voucherid='" + TxtBillNo.Text + "' and trantype='Sale Return' and [CompanyId]=" + Master.companyId + "", con);
                    cmd3.ExecuteNonQuery();
                    MessageBox.Show("Invoice Delete Successfully");
                    this.Close();
                    datewiseSaleReturn.bindgrid();
                }
                else
                {
                    TxtBillNo.Focus();
